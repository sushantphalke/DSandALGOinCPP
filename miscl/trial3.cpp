...
void executive(int id) {
  while (true) {
    ...
    // Implement logic to check if the executive is available for new clients
    {
      ...
      time_t now = time(0);
      tm* dt = localtime(&now);
      cout << dt->tm_hour << ":" << dt->tm_min << ":" << dt->tm_sec << " Executive " << id << " is available.\n";
    }
  }
}
...
void client(int id, int execId) {
  // Simulate sending a request to the controller
  {
    ...
    time_t now = time(0);
    tm* dt = localtime(&now);
    cout << dt->tm_hour << ":" << dt->tm_min << ":" << dt->tm_sec << " Client " << id << " requests assistance from Executive " << execId << ".\n";
  }
  ...
  // Simulate closing the conversation and releasing the executive
  {
    ...
    time_t now = time(0);
    tm* dt = localtime(&now);
    cout << dt->tm_hour << ":" << dt->tm_min << ":" << dt->tm_sec << " Client " << id << " conversation with Executive " << execId << " completed.\n";
  }
}
